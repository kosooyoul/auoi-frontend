class HanulseEnergyEffectRenderer {
	render(context, _timeOffset) {
		context.save();

		context.translate(0, -20);

		this.drawMainBall(context, 0, 1.5, 2.0);
		this.drawMainBall(context, 0, 1.2, 1.0);
		this.drawMainBall(context, 0, 0.8, 2.4);
	...
	drawMainBall(context, timeOffset, timeScale, scale) {
		...
		var ratio = ((tick * timeScale) % 2000) / 2000;

		var curvedRatio = Math.sin(ratio * Math.PI);

		var alpha = (1 - (curvedRatio * 2 - 1) * (curvedRatio * 2 - 1)) * 0.4 + 0.2;
	...
	drawEnergyBall(context, timeOffset, timeScale, scale, rotate) {
		...
		var alpha = (1 - (curvedRatio * 2 - 1) * (curvedRatio * 2 - 1)) * 1;
